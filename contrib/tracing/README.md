...
Tracing (USDT) support for Waltercoin Core on Linux using. For more information on
USDT support in Waltercoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `waltercoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `waltercoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Waltercoin Core USDT documentation] on how to list available tracepoints in your
`waltercoind` binary.

[Waltercoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/waltercoind
```
...
Waltercoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/waltercoind
```
...
In a different terminal, starting Waltercoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/waltercoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/waltercoind
```
