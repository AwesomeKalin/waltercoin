# Bootstrappable Waltercoin Core Builds

This directory contains the files necessary to perform bootstrappable Waltercoin
Core builds.
...
To build Waltercoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Waltercoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/waltercoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/waltercoin-core/guix.sigs).
