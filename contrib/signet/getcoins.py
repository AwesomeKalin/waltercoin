...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing waltercoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='waltercoin-cli', help='waltercoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Waltercoin address to which the faucet should send')
...
parser.add_argument('waltercoin_cli_args', nargs='*', help='Arguments to pass on to waltercoin-cli (default: -signet)')

args = parser.parse_args()

if args.waltercoin_cli_args == []:
    args.waltercoin_cli_args = ['-signet']


def waltercoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.waltercoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = waltercoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = waltercoin_cli(['getnewaddress', 'faucet', 'bech32'])
