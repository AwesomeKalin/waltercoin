#!/usr/bin/env python3
# Copyright (c) 2014-2021 The Waltercoin Core developers
...
TMPDIR_PREFIX = "waltercoin_func_test_"
...
class WaltercoinTestMetaClass(type):
    """Metaclass for WaltercoinTestFramework.

    Ensures that any attempt to register a subclass of `WaltercoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'WaltercoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("WaltercoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("WaltercoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class WaltercoinTestFramework(metaclass=WaltercoinTestMetaClass):
    """Base class for a waltercoin test script.

    Individual waltercoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave waltercoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop waltercoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use waltercoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_waltercoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "waltercoind" + config["environment"]["EXEEXT"],
        )
        fname_waltercoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "waltercoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_waltercoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "waltercoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.waltercoind = os.getenv("WALTERCOIND", default=fname_waltercoind)
        self.options.waltercoincli = os.getenv("WALTERCOINCLI", default=fname_waltercoincli)
        self.options.waltercoinutil = os.getenv("WALTERCOINUTIL", default=fname_waltercoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: waltercoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("WaltercoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'waltercoind', self.options.waltercoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'waltercoin-cli', self.options.waltercoincli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                waltercoind=binary[i],
                waltercoin_cli=binary_cli[i],
                version=versions[i],
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.waltercoinconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a waltercoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple waltercoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as waltercoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    waltercoind=self.options.waltercoind,
                    waltercoin_cli=self.options.waltercoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in waltercoin.conf
    ...
    def skip_if_no_waltercoind_tracepoints(self):
        """Skip the running test if waltercoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("waltercoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_waltercoind_zmq(self):
        """Skip the running test if waltercoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("waltercoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if waltercoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("waltercoin-wallet has not been compiled")

    def skip_if_no_waltercoin_util(self):
        """Skip the running test if waltercoin-util has not been compiled."""
        if not self.is_waltercoin_util_compiled():
            raise SkipTest("waltercoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if waltercoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("waltercoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether waltercoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether waltercoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_waltercoin_util_compiled(self):
        """Checks whether waltercoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_WALTERCOIN_UTIL")
