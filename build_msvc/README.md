Building Waltercoin Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build Waltercoin Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Waltercoin Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Waltercoin Core with the GUI, a static build of Qt is required.
...
To build Waltercoin Core without Qt, unload or disable the `waltercoin-qt`, `libwaltercoin_qt` and `test_waltercoin-qt` projects.
...
```cmd
msbuild build_msvc\waltercoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/waltercoin.sln` file in Visual Studio.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make Waltercoin Core more secure. When building Waltercoin using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `waltercoind` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/waltercoind.exe
```
