# PSBT Howto for Waltercoin Core

Since Waltercoin Core 0.17, an RPC interface exists for Partially Signed Waltercoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/waltercoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Waltercoin transactions that are not fully signed
...
#### Multisig with multiple Waltercoin Core instances
...
Waltercoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `waltercoin-cli` is used.
