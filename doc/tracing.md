# User-space, Statically Defined Tracing (USDT) for Waltercoin Core

Waltercoin Core includes statically defined tracepoints to allow for more
...
**Note**: Waltercoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Waltercoin Core
...
Multiple tools can list the available tracepoints in a `waltercoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Waltercoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/waltercoind
stap net        outbound_message 0x0000000000107c05 /src/waltercoind
stap validation block_connected  0x00000000002fb10c /src/waltercoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Waltercoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/waltercoind | grep NT_STAPSDT -A 4 -B 2
