# CJDNS support in Waltercoin Core

It is possible to run Waltercoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Waltercoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Waltercoin network and individual nodes.
...
before it will work with your Waltercoin Core node. This is described in steps
...
## Run Waltercoin Core with CJDNS

Once you are connected to the CJDNS network, the following Waltercoin Core
...
When enabled, this option tells Waltercoin Core that it is running in an
...
IPv6 local network. This helps Waltercoin Core perform better address management:
...
CJDNS support was added to Waltercoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `waltercoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/waltercoin/waltercoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Waltercoin Core

There are several ways to see your CJDNS address in Waltercoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `waltercoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `waltercoin-cli getpeerinfo`).
