...
#include <qt/waltercoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Waltercoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new WaltercoinAddressEntryValidator(parent));
    widget->setCheckValidator(new WaltercoinAddressCheckValidator(parent));
}
...
bool parseWaltercoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no waltercoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!WaltercoinUnits::parse(WaltercoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseWaltercoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseWaltercoinURI(uriInstance, out);
}

QString formatWaltercoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("waltercoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WaltercoinUnits::format(WaltercoinUnit::BTC, info.amount, false, WaltercoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openWaltercoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", WALTERCOIN_CONF_FILENAME));
    ...
    /* Open waltercoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Waltercoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Waltercoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Waltercoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("waltercoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a waltercoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Waltercoin\n";
        else
            optionFile << strprintf("Name=Waltercoin (%s)\n", chain);
