...
/** Call node.updateRwSetting() with Waltercoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Waltercoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/waltercoin/waltercoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because waltercoin 22.x will not try to read these.
...
//! Convert enabled/size values to waltercoin -prune setting.
...
//! Get pruning enabled value to show in GUI from waltercoin -prune setting.
...
//! Get pruning size value to show in GUI from waltercoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayWaltercoinUnit")) {
        settings.setValue("DisplayWaltercoinUnit", QVariant::fromValue(WaltercoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayWaltercoinUnit");
    if (unit.canConvert<WaltercoinUnit>()) {
        m_display_waltercoin_unit = unit.value<WaltercoinUnit>();
    } else {
        m_display_waltercoin_unit = WaltercoinUnit::BTC;
        settings.setValue("DisplayWaltercoinUnit", QVariant::fromValue(m_display_waltercoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if waltercoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_waltercoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<WaltercoinUnit>() == m_display_waltercoin_unit) return;
    m_display_waltercoin_unit = new_unit.value<WaltercoinUnit>();
    QSettings settings;
    settings.setValue("DisplayWaltercoinUnit", QVariant::fromValue(m_display_waltercoin_unit));
    Q_EMIT displayUnitChanged(m_display_waltercoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/waltercoin/waltercoin/pull/8273
    ...
    // (https://github.com/waltercoin-core/gui/issues/567).
