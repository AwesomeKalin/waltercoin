// Copyright (c) 2015-2021 The Waltercoin Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<WaltercoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = WaltercoinUnits::formatWithUnit(unit, expected_balance, false, WaltercoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_waltercoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_waltercoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_waltercoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    // Amount input
    WaltercoinAmountField* amountInput = receiveCoinsDialog.findChild<WaltercoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("waltercoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_waltercoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
