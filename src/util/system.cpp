...
const char * const WALTERCOIN_CONF_FILENAME = "waltercoin.conf";
const char * const WALTERCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //waltercoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", WALTERCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Waltercoin
    // macOS: ~/Library/Application Support/Waltercoin
    // Unix-like: ~/.waltercoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Waltercoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", WALTERCOIN_CONF_FILENAME);
